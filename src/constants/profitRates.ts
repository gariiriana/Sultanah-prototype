...
export type ProfitRole = keyof typeof PROFIT_RATES;

/**
 * Get profit amount based on referrer role
 */
export function getProfitAmount(referrerRole: string): number {
    if (referrerRole === 'alumni') {
        return PROFIT_RATES.alumni;
    } else if (referrerRole === 'agen' || referrerRole === 'brand_ambassador') {
        return PROFIT_RATES.brand_ambassador;
    }
    return 0; // No profit for other roles
}

/**
 * Format profit amount to Indonesian Rupiah
 */
