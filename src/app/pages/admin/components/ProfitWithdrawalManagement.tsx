...
const ProfitWithdrawalManagement: React.FC = () => {
  ...
  const loadWithdrawals = async () => {
    try {
      setLoading(true);
      const querySnapshot = await getDocs(collection(db, 'profitWithdrawals'));
  ...
  const handleApprove = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'profitWithdrawals', selectedWithdrawal.id), updateData);
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      await updateDoc(doc(db, 'profitWithdrawals', selectedWithdrawal.id), {
        status: 'rejected',
        processedDate: Timestamp.now(),
        note: note,
      });
...
export default ProfitWithdrawalManagement;
