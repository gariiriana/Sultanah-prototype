...
const PaymentForm: React.FC<PaymentFormProps> = ({ onBack, onViewStatus }) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (formData.paymentMethod === 'Bank Transfer') {
      ...
      if (!formData.ewalletProvider || !formData.phoneNumber) {
    ...
    try {
      const paymentData = {
        ...
        installmentPlan: formData.paymentType === 'installment' ? {
          ...
          paidInstallments: formData.currentInstallment - 1,
      ...
      toast.success('Payment submitted successfully! Redirecting to payment status...');
      ...
      // Redirect to payment status page after 1.5 seconds
      setTimeout(() => {
        if (onViewStatus) {
          onViewStatus();
        } else {
  ...
  const selectedPackage = packages.find(pkg => pkg.id === formData.booking);
  const packagePrice = selectedPackage?.price || 0;
  const installmentAmount = formData.paymentType === 'installment' && formData.totalInstallments > 0
    ? Math.ceil(packagePrice / formData.totalInstallments)
    : packagePrice;
  const totalPaid = installmentAmount * (formData.currentInstallment - 1);
  const remainingAmount = packagePrice - totalPaid - (formData.amount ? parseFloat(formData.amount) : 0);
        ...
        >
          ...
          <div className="bg-gradient-to-r from-[#C5A572] to-[#D4AF37] rounded-2xl p-8 text-white">
            ...
            <p className="text-white/90">
              Fill in your payment details and upload proof of transfer
            </p>
          </div>
        </motion.div>
        ...
        >
          ...
          <div className="p-8 border-b border-gray-100">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                          ...
                          <div className="flex-1">
                            ...
                            <div className="space-y-1.5 text-sm">
                              <div className="flex justify-between">
                                ...
                                <span className="font-semibold text-blue-900">Rp {packagePrice.toLocaleString('id-ID')}</span>
                              </div>
                              <div className="flex justify-between">
                              ...
                              <div className="flex justify-between">
                                ...
                                <span className="font-semibold text-blue-900">Rp {totalPaid.toLocaleString('id-ID')}</span>
                              </div>
                              <div className="border-t border-blue-200 pt-1.5 flex justify-between">
                                ...
                                <span className="font-bold text-blue-900">Rp {Math.max(0, remainingAmount).toLocaleString('id-ID')}</span>
                              </div>
                            </div>
                          </div>
                        </div>
              ...
              {/* Amount */}
              <div className={formData.paymentType === 'full' ? '' : 'md:col-span-2'}>
                <label className="block text-sm font-medium text-gray-700 mb-2">
