...
import { ArrowLeft, Package as PackageIcon, Star, Clock, Calendar, Users, Check, Sparkles, Crown } from 'lucide-react';
...
type CategoryTab = 'reguler' | 'promo' | 'limited-edition';
...
const PackageBrowsePage: React.FC<PackageBrowsePageProps> = ({
    ...
    const [activeTab, setActiveTab] = useState<CategoryTab>('reguler');
    const [filteredPackages, setFilteredPackages] = useState<Package[]>([]);

    useEffect(() => {
        // Filter packages by category, default to 'reguler' if no category set
    ...
    const tabs: CategoryTab[] = ['reguler', 'promo', 'limited-edition'];
                                        ...
                                        <div className="relative h-48">
                                            ...
                                            {pkg.type && (
                                                <div className="absolute top-3 right-3">
                                                    <span className="px-3 py-1 rounded bg-[#D4AF37] text-white text-xs font-bold uppercase">
                                                        {pkg.type}
                                                    </span>
                                                </div>
                                            )}
                                            ...
                                            {pkg.packageCategory && pkg.packageCategory !== 'reguler' && (
                                                <div className="absolute top-3 left-3">
                                                    <span className={`
                            px-3 py-1 rounded text-white text-xs font-bold uppercase flex items-center gap-1
                            ${pkg.packageCategory === 'promo' ? 'bg-orange-500' : 'bg-purple-600'}
                          `}>
                                                        {pkg.packageCategory === 'promo' ? <Sparkles className="w-3 h-3" /> : <Crown className="w-3 h-3" />}
